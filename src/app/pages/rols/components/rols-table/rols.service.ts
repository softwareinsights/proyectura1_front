...
export class RolsService {
    ...
    constructor(
       ...
       findById = ( id ) : Observable<RolsResponseInterface> => {
           return this._http.get(`${this.endPoint}/${id}`, this.options)
               .map((response: Response) => response.json())
               .catch(this.handleError);
       }
       update = ( rol: RolsInterface ) : Observable<RolsResponseInterface> => {
           return this._http.patch(this.endPoint, rol, this.options)
               .map((response: Response) => response.json())
               .catch(this.handleError);
       }
       ...
       insert = ( rol: RolsInterface ) : Observable<RolsResponseInterface> => {
           return this._http.post(this.endPoint, rol, this.options)
               .map((response: Response) => response.json())
               .catch(this.handleError);
       }
